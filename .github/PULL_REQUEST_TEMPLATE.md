...
<!-- 🔴 必填：简要说明本次改动的技术选型与核心实现逻辑（提交规则 3.3 第 ③ 项） -->

### 技术选型
- 用到了哪些库/框架？为什么选它？（对比过的备选方案及放弃理由）
- 是否引入新依赖？是否已加入 `requirements.txt` / `package.json`？

### 核心设计
- 关键数据结构 / 接口 / 状态机是什么样？
- 关键算法 / 业务流程怎么走？（可贴伪代码或时序图）
...
- [ ] PR 描述已包含 4 要素：标题、功能描述、实现思路、测试方式（规则 3.3）
- [ ] 已在本地自测通过
- [ ] 主分支（main）合并后保持可运行状态
...
- [ ] 新引用的第三方库 / 框架已在 README 中列举（规则 3.2）
